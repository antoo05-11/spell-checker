...
class CustomTokenizer:
    ...
    def sort(self):
        ...
        self.index_word = dict(
            enumerate(dict(sorted(self.word_index.items())), start=self.start)
        )
        ...
        for index, token in enumerate(line_tokens):
            ...
            for c_index, char in enumerate(token):
                if char in self.filters or (self.filter_nums and char.isdigit()):
                    filtered_tokens += (
                        [char, ""] if c_index != len(token) - 1 else [char]
                    )
                else:
    ...
    def fit_on_texts(self, lines: typing.List[str]):
        ...
        self.word_index = {
            ...
            for value, key in enumerate(
                [self.start_token, self.end_token, self.split] + self.filters
            )
        }
        ...
        for line in tqdm(lines, desc="Fitting tokenizer"):
            ...
            self.max_length = max(
                self.max_length, len(line_tokens) + 2
    ...
    def detokenize(self, sequences: typing.List[int], remove_start_end: bool = True):
        ...
        for sequence in sequences:
            ...
            for token in sequence:
                if token == 0:
                    break
                if remove_start_end and (
                    token == self.start_token_index or token == self.end_token_index
                ):
                    continue
    ...
    def texts_to_sequences(
        self, lines: typing.List[str], include_start_end: bool = True
    ):
        ...
        for line in lines:
            line_tokens = self.split_line(line)
            sequence = [
                self.word_index[word] for word in line_tokens if word in self.word_index
            ]
